...
const CreateSupervision = ({ newId }) => {
  ...
  const onSubmit = () => {
    ...
    axios
      .post('/api/checklist', postChecklistData)
      .then(function (response) {
        setIsSubmitting(false);
        toast({
          title: 'Supervisão cadastrada',
          status: 'success',
          duration: 3000,
          isClosable: true,
          onCloseComplete: () =>
            router.push(`/checklist/${response.data.idCreate}`),
        });
      })
  ...
  return (
    <>
      <Head>
        <title>Milk Check - Nova supervisão</title>
      </Head>
      ...
      >
        ...
        <VStack w="full" px="8">
          <FormControl isRequired isInvalid={validation.type === false}>
            ...
            <RadioGroup defaultValue="BPA" h="32px" mt="4">
              <Radio
                onChange={(e) =>
                  handleChange({ name: e.target.name, value: e.target.value })
                }
...
export async function getServerSideProps() {
  ...
  return {
    props: { newId: Math.max(...supervisions.map((sup) => sup._id)) + 1 },
  };
}

export default CreateSupervision;
